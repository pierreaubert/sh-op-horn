...
class HelmhotzSolver(fs.PdeConstraint):
    """"Solve the Helmholz equation."""

    def __init__(self, mesh, ndim, state_conf, freqs):
        super().__init__()
        self.mesh = mesh
        self.ndim = ndim
        self.c_air = state_conf["c_air"]
        self.rho_air = state_conf["rho_air"]
        self.g_inlet = state_conf["g_inlet"]
        self.g_outlet = state_conf["g_outlet"]
        self.p_A_in = state_conf["p_A_in"]
        self.freqs = freqs
        self.failed_to_solve = False  # when self.solver.solve() fail
        self.V = fd.FunctionSpace(self.mesh, "CG", 1)
        self.solution = []

    def solve_helmholz(self, freq, sol):
        u = fd.TrialFunction(self.V)
        v = fd.TestFunction(self.V)
        kappa = fd.Constant(2 * pi * freq / self.c_air)
        a_prod = fd.inner(fd.grad(u), fd.grad(v)) - kappa ** 2 * fd.inner(u, v)
        # right hand side
        a_in = self.rho_air * 1j * kappa * fd.inner(u, v)
        a_out = (self.rho_air * 1j * kappa + 1 / 2) * fd.inner(u, v)
        a = a_prod * fd.dx + a_in * fd.ds(self.g_inlet) + a_out * fd.ds(self.g_outlet)
        L = (
            -2
            * self.p_A_in
            * self.rho_air
    ...
    def solve(self):
        ...
        for freq in self.freqs:
            self.solve_helmholz(freq, sol)
            self.solution.append(self.sol.copy(deepcopy=True))
            PETSc.Sys.Print("{:.1f}hz ".format(freq), end="", flush=True)


class HornObjective(fs.ShapeObjective):
    """Minimuse J integral of reflections on inlet"""

    def __init__(self, pde_solver: HelmhotzSolver, config, *args, **kwargs):
    ...
    def value_form(self):
        ...
        results = self.pde_solver.solution
        g_inlet = self.conf["g_inlet"]
        r0 = list(results.values())[0]
        length_in = abs(fd.assemble(r0 / r0 * fd.ds(g_inlet)))
        reflections = [
            abs(abs(fd.assemble(abs(s_hz) * fd.ds(g_inlet)) / length_in) - 1)
        ...
        return np.linear.norm(reflections)
